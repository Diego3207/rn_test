...
export class EditUsersComponent 
{
	...
	{				
		...
		forkJoin([branchCatalogObservable, rolCatalogObservable] ).subscribe(([ dataBranches, dataRoles]) => 
		{
			...
			{				
				...
				for (let i=0; i < res.data.userBranches.length; i++)
				{
					this.addRow('branches');
				} 
    ...
    removeRow(type,object,index:number)
	{
        ...
        if(type == 'branches')
        {
			...
			if(object.value.id !== null) //si el elemento tenia id, se reserva para eliminar del arreglo
                this.deletedBranches.push(object.value.id);
        }
    }
	...
	updateForm(img?:string)
	{
		...
		(data:any)=>{
			...
			this.updateBranches(this.form.value.id); //setea el id de la forma original
		},
		(error:any)=>{
	...
	updateBranches(id:string)
	{
		...
		if( userBranches.length>0)
		{
			...
			userBranches.forEach(obj =>  //recorro el arreglo de elementos
			{
				if(obj['id'] == null) //si el id no existe, el elemento es nuevo
				{
					obj['userBranchUser'] = id;
					const ptt = this.userBranchService.create(obj).pipe(
						catchError((error) => 
						{
							this.messageService.add({ life:5000, key: 'msg', severity: 'error', summary: "Error al guardar elemento de sucursal", detail:error.message });
							return of(null);
						})
					);				
					peticiones.push(ptt);
				...
				else //de lo contrario el elemento ya existia y requiere una actualizacion
				{
					const ptt = this.userBranchService.update(obj).pipe(
						catchError((error) => 
						{
							this.messageService.add({ life:5000, key: 'msg', severity: 'error', summary: "Error al guardar elemento de sucursal", detail:error.message });
							return of(null);
			...
			this.deletedBranches.forEach(id => //se eliminan los elementos removidos
			{
                const ptt = this.userBranchService.disable(id).pipe(
					catchError((error) => 
					{
						this.messageService.add({ life:5000, key: 'msg', severity: 'error', summary: "Error al eliminar la sucursal", detail:error.message });
						return of(null);
					})
				);				
				peticiones.push(ptt);
